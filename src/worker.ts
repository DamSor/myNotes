...
async function runWeeklySummaries(env: Env): Promise<void> {
  try {
    ...
    const result = await supabase.from("notes").select("user_id").gt("created_at", windowStart).limit(10_000);
    ...
    // eslint-disable-next-line no-console -- intentional: catch-all for infra-level failures
    console.error("weekly-summary: unexpected error", e);
  }
}
